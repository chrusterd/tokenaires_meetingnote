...
회의록 DB에는 `회의명`(Title), `날짜`(Date), `내용`(Text), `참여자`(다중 선택), `상태`(선택: 진행 예정 / 진행 중 / 완료)가 필요합니다.

회의명은 `[7/29] 간단 요약 제목` 형식으로 저장됩니다. 날짜에는 회의 날짜가 아니라 Notion에 저장(승인)한 시각이 시:분까지 들어갑니다. 상태는 검토 화면에서 사람이 직접 고르며 기본값은 "완료"입니다.
